...
writeFile(path.join(writePath, 'export-info'), {
  contentVersion: 2,
  logsPath: writePath,
});
...
const createMetaData = async (assetData) => {
  ...
  await writeFile(path.join(writePath, 'assets', 'metadata'), data);
};
...
const handleAssets = async () => {
  ...
  for (let i = 0; i < Object.keys(chunks).length; i++) {
    ...
    await writeFile(
      path.join(writePath, 'assets', `${chunkId}-assets`),
      JSON.stringify(
        {
          ...chunks[chunkId],
        },
  ...
  await writeFile(
    path.join(writePath, 'assets', 'assets'),
