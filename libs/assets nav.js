  ...
  saveAsset: function (assetUrl, retryCount) {
    ...
    return when.promise(async function (resolve, reject) {
      try {
        ...
        // }
        for (const item of Object.values(assetUrl)) {
          await processUrl(item?.image);
        }
  ...
  getAsset: function (attachments) {
    ...
    return when.promise(async function (resolve, reject) {
      ...
      try {
        ...
        for (const assetArray of Object.values(attachments)) {
          if (typeof assetArray === 'object') {
            for (const item of Object.values(assetArray)) {
              if (item?.carouselItems) {
                // console.log('item.carouselItems', item.carouselItems);
                await self.saveAsset(item?.carouselItems);
              }
            }
          }
          // console.log(typeof assetArray);
        }

        // for (const assetArray of attachments) {
        //   if (assetArray === 'carouselItems') {
  ...
  getAllAssets: function (templatePaths) {
    ...
    return when.promise(async function (resolve, reject) {
      try {
        ...
        let assetArrays = [data?.menuNavigation];
